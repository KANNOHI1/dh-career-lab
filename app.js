  ...
  },
  {
    ...
    qs: [
      ...
      { id:'certs', t:'持っている認定・専門資格は？', type:'checkbox',
        ...
        // 日本歯科衛生士会の認定は最大手（訪問・予防トラックの要件にも登場する）のに
        // 選択肢に無く「その他」送りになっていたため 2026-08-28 に追加
        opts:['歯周病の認定','ホワイトニングコーディネーター','インプラントの認定','矯正の認定','麻酔系の認定（認定麻酔衛生士など）','日本歯科衛生士会の認定（在宅療養指導・口腔機能管理など）','その他'] },
...
// ---------- あなたの棚卸し ----------
// 回答した事実を公的統計と並べる。まとめの言い切りの根拠がここに揃う。
// 統計の付録（数字の裏側）ではなく本人のことなので、結果の前半に置く。
...
function result() {
  ...
  // ================= 第1段: あなたはいまここ =================
  // 数の羅列（実務15年 / 領域2つ / 記録3件）では「持っているもの」の中身が伝わらない。
  // 回答から導出できる事実だけを、言い切りの行にして最初に見せる。
  // 希少性・市場との比較はここでは言わない（無出典の主張になる）。回答の言い直しに徹する。
  // 道の名前は出さない（名前が出るのは下のランキング1箇所だけ）。
  const fy = A.fieldYears || {};
  const deep = (A.fields || []).filter(f => fy[f] >= 3);
  const lines = [];
  if (A.years) lines.push('実務 <strong>' + A.years + ' 年</strong>');
  if (deep.length >= 2) {
    lines.push(deep.map(f => f + ' ' + fy[f] + '年').join('・') + ' — ' +
      (deep.length === 2 ? 'どちらも' : 'いずれも') + '「かじった」ではなく主戦場');
  } else if (deep.length === 1) {
    lines.push(deep[0] + ' ' + fy[deep[0]] + '年 — 主戦場として通ってきた');
  }
  if (A.handedness === '左') {
    lines.push('左利きの解決策を、教わる相手がいないまま自分で作ってきた');
  }
  ...
  // 棚卸しは統計の付録ではなく本人のこと。「数字の裏側」から出して、選択肢より先に置く。
  // 既定たたみは維持（開くかどうかは本人が決める）。
  w.appendChild(foldSection('あなたの棚卸し', '年数・領域・資格・まわりからの扱い', inventorySection));
  ...
  if (notes.length) {
    ...
    // 書かずに通り過ぎた人（それが普通）に、戻る道だけ示しておく
    w.appendChild(card('unknown',
      'STEP 5 の「工夫していること」は、あとからでも書き足せます。' +
      '書いたことが、そのまま「人に教えられること」の材料になります。' +
      '下の「戻って直す」を押すと、ちょうどその画面に戻ります。'));
  }

  // ================= まず、ひとつだけ =================
  // 11個の箱それぞれに行動があっても、ページ全体の「次の一歩」がないと決められない。
  // いちばん上の道の今月の行動だけを取り出して、ここで終わる。
  // 道の名前の再掲はこの1箇所だけ許す（目的ラベル付き・HK 2026-08-28 プラン承認）。
  if (matchReady() && ranked[0] && ranked[0].actions && ranked[0].actions.length &&
      ranked[0].verdict !== 'not-recommended') {
    const t0 = ranked[0], a0 = t0.actions[0];
    w.appendChild(card('card flag',
      '<span class="tag">まず、ひとつだけ</span>' +
      '<p style="margin:.2rem 0 0">いちばん上に出た「' + t0.name + '」の、今月の一歩はこれです。</p>' +
      '<ol class="plan" style="margin-top:.6rem"><li><span class="when">' + a0.when + '</span>' + a0.text +
